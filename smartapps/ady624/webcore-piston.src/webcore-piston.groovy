		...
		if (skip) {
			...
			if (params.size()) {
				if (myres && delay) { //not supported in hubitat
					...
					msg.m = "Queued physical command [${device.label ?: device.name}].$command($params, [delay: $delay])"
				} else {
				...
				if (myres && delay) { //not supported in hubitat
					...
					msg.m = "Queued physical command [${device.label ?: device.name}].$command([delay: $delay])"
				} else {
					//device."$command"()
